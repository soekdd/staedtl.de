...
function getCurrent(arr) {
  Math.seedrandom(new Date().getYear());
  const disordered = shuffle([...arr]);
  const day = Math.floor(new Date().getTime() / (1000 * 60 * 60 * 24)) + 1;
  return disordered[day % disordered.length];
}
...
function setISO(iso) {
  ...
  if (level == 1) {
    ...
    let today = new Date();
    currentCity = getCurrent(dataRandom);
...
function setInfoLevel(level) {
  if (level > 1) {
    try {
      document
        ...
        .setAttribute(
          "placeholder",
          (level < maxLevel ? level + "." : "letzter") + " Versuch"
        );
    } catch (e) {}
    document.getElementById("eu").style = "display:none";
    document.getElementById("de").style = "display:none";
  ...
  if (levels.includes("alt")) {
    document.getElementById("alt").innerHTML =
      "<h4>Höhe über N.N.</h4><p>" + currentCity.a + " m</p>";
  }
  if (levels.includes("pop")) {
    document.getElementById("pop").innerHTML =
      "<h4>Einwohnerzahl:</h4><p>" +
      parseInt(currentCity.i).toLocaleString("de-DE") +
      "</p>";
  }
  if (levels.includes("plz") && game == "de") {
    if (Array.isArray(currentCity.z))
      document.getElementById("plz").innerHTML =
        "<h4>Postleitzahlen:</h4><p>" + currentCity.z.join("-") + "</p>";
    else
      document.getElementById("plz").innerHTML =
        "<h4>Postleitzahl:</h4><p>" + currentCity.z + "</p>";
  }
  if (levels.includes("plz") && game == "eu") {
    document.getElementById("plz").innerHTML =
      "<h4>Land:</h4><p>" + currentCity.c + "</p>";
  }
  if (levels.includes("wappen")) {
    document.getElementById("wappen").src =
      handleURL(currentCity.f) + "?width=300px";
  }
  if (levels.includes("state")) {
    document.getElementById("state").innerHTML =
      "<h4>Verwaltungseinheit:</h4><p>" + currentCity.s.split(",")[0] + "</p>";
  }
  ...
  if (levels.includes("photo")) {
    document.getElementById("header").style =
      "background-image: url(" + handleURL(currentCity.p) + "?width=1200px)";
    document.getElementById('copyright').style = "display:block";
    document.getElementById('copyright').innerHTML = "Copyright: "+( currentCity.r != '' ? currentCity.r : 'unbekannt' );
  }
  if (levels.includes("location")) {
    document.getElementById("location").src =
      handleURL(currentCity.l) + "?width=400px";
  }
}
var currentCity;
...
function getIndex(label) {
  for (let index in data)
    if (data[index].n.toUpperCase() == label.toUpperCase()) return index;
  return -1;
}
function nextGame() {
  if (game == "de") document.getElementById("nowEU").style = "";
  else document.getElementById("nowDE").style = "";
}
...
function nextTry() {
  ...
  if (currentCity.n.toUpperCase() == input) {
    document.getElementById("inputArea").innerHTML =
      '<h2 class="won">GEWONNEN!</h2><p>' + spruch[level] + "</p>";
    setInfoLevel(99);
    fireworks = new Fireworks.default(document.getElementById("firework"), {
      explosion: maxLevel - level + 1,
      particles: 10 * (maxLevel - level + 1),
    });
...
function checkOK() {
  let input = document.getElementById("input").value.toUpperCase();
  let found = false;
  for (let city of data) found = found || city.n.toUpperCase() == input;
...
function updateTries() {
  ...
  for (let city of tries) {
    count++;
    let tGeo = city.g;
    let geo = window.geodesic.Geodesic.WGS84.Inverse(
      tGeo[1],
      tGeo[0],
      cGeo[1],
      cGeo[0]
...
function proposal() {
  ...
  for (let f of fuse.search(input).slice(0, 5)) {
    s +=
      '<div class="pLine" onclick="setProp(\'' +
      f.item.n +
      "')\" >" +
      f.item.n +
      "</div>";
...
function impressum(action) {
  if (action == "show") {
    ...
    document.getElementById("impressum").style = "display:none";
    if (level == 1) document.getElementById("privacy").style = "";
  }
}
...
function info(action) {
  if (action == "show") {
    ...
    document.getElementById("minDe").innerHTML =
      "&gt;" +
      Math.trunc(Math.min(...datade.map((d) => d.i)) / 1000) +
      " TEinw.";
    document.getElementById("minEu").innerHTML =
      "&gt;" +
      Math.trunc(Math.min(...dataeu.map((d) => d.i)) / 1000) +
      " TEinw.";
  } else {
    document.getElementById("infoField").style = "display:none";
    if (level == 1) document.getElementById("privacy").style = "";
